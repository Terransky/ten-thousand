...
class GameLogic:
    """class manages methods for rolling dice and calculating score"""
    ...
    def calculate_score(dice_roll_tuple):
        ...
        We used a nested if/else statements to account for different configurations/combinations of dice set.
        """
        dice_roll = list(dice_roll_tuple)
#         # print(dice_roll)
        score = 0
        # count stores the number of times a specific score has been rolled in the list;
        # for instance, if the 6 dice are rolled an d3 of the faces are 1's, then index 0 of count will be 3.
        # Most references to count will be shifted back by one; ie. to check how many times 4 has been rolled,
        # you look at the (four minus one)th index. This is because you can't roll a 0
        count = [0, 0, 0, 0, 0, 0]
        ...
        # This else has the math for the other cases that use math to compute
        # the scores from having multiple things in certain values
        else:
            for face in range(1, 7):
                number = count[face-1]
                # The NUMBER of dice with the face value integer we are looking for
                # (aka 4 die that are rolled with 5's face-up)
                ...
                if face in range(1, 7):
                    if face == 1:
                        ...
                        if number >= 3:
                            ...
                            # print(face, score)
                        # For other numbers, they only get scores if there are more than three of them rolled.
                        # If there are three of these numbers then the score is the face*100*amount of the number -2
